...
monkey_lcm = lcm(*[test[0] for test in tests.values()])
...
for r in range(rounds):
    for m in monkeys:
        while items[m]:
            inspections[m] += 1
            # get lcm of all divisable by values so that using mod on new_item won't affect the calculation of the new monkey
            new_item = operations[m](items[m].pop(0)) % monkey_lcm
